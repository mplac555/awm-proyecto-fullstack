...
module.exports.createAlerta = (request, response) => {
    ...
    const { alertaDate, alertaHour, brigName, brigLastname, carPlate, alertaIncident, alertaDescription } = request.body; // Extraemos el adminName, adminLastname, adminDNI, adminMail, adminPassword y adminPhone del cuerpo de la solicitud.
    const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
    if (!dateRegex.test(alertaDate)) {
        return response.status(400).json({ message: 'El formato de la fecha debe ser "YYYY-MM-DD".' });
    }
    Alerta.create({
        alertaDate,
        alertaHour,
...
module.exports.getAllAlertas = (_, response) => {
    Alerta.find({}).sort({ alertaDate: 1, alertaHour: 1 }) // Ordenamos por 'adminName' y 'adminLastname' en orden alfabético ascendente (1).
