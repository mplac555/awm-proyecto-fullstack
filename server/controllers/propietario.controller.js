...
module.exports.getAllOwners = (_, response) => {
    Owner.find({}).sort({ ownerName: 1, ownerLastname: 1 }) // Ordenamos por 'ownerName' y 'ownerLastname' en orden alfabético ascendente (1).
...
module.exports.addCarToOwner = (request, response) => {
    ...
    .then(newCar => {
        ...
        Owner.findByIdAndUpdate(ownerId, { $push: { ownerCars: newCar._id } }, { new: true })
...
module.exports.deleteCarFromOwner = (request, response) => {
    ...
    Owner.findById(ownerId)
        .then(owner => {
            ...
            owner.ownerCars = owner.ownerCars.filter(car => car.toString() !== carId);
