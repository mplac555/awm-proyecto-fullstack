...
const BrigadeScan_Mobile=({navigation, route})=>{
  ...
  const {name, last, email} = route.params;
  ...
  const alertPositive = () =>{
    ...
    const registro = {alertaDate: fechaA , alertaHour: horaA, brigName: name, brigLastname: last
    , carPlate: placa ,alertaIncident:'Verificado',alertaDescription: 'Sin Novedad' };

    console.log(registro);
      axios.post("http://"+host+":8000/api/alerta/new", registro).then(res => {
      console.log('Alerta enviada');
      //console.log(res);
      navigation.navigate('BrigadeProfile_Mobile', {userName: name, email: email})
        ...
        <View style={styles.container}>
          ...
          <Box style={styles.box}>
            ...
            (
             ...
             <CameraQr
             onTextChange = {handlePlacaChnage}
             ></CameraQr>
            )}
            
            
            <Button title={conScaner==false ? ('Escanear'):('Cancelar')} 
            style={styles.button}  backgroundColor ={'rgba(33,150,243,255)'}
              onPress={ControlScanner}
            />
          </Box>
          <Box style={styles.butonsPanel}>
                 <Button title="Verificar" style={styles.button}  backgroundColor ={'rgba(46,125,50,255)'}
                     onPress={()=>alertPositive()}
                />
                <Button title="Reportar" style={styles.button}  backgroundColor ={'rgba(33,150,243,255)'}
                     onPress={()=> navigation.navigate('BrigadeIncident_Mobile', {name: name, last: last, email: email})}
                />
                <Button title="Regresar" style={styles.button} backgroundColor ={'rgba(211,47,47,255)'}
                     onPress={()=> navigation.navigate('BrigadeProfile_Mobile', {userName: name, email: email})}
                />
            </Box>
