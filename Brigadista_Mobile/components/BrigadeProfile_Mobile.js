...
const BrigadeProfile_Mobile = ({navigation, route})=>{
    ...
    useEffect(()=>{
      axios.get('http://'+host+':8000/api/brigadista/'+userName+'/'+email)
      .then(res => {
        setBrigade(res.data);
    ...
    return(
        <View style={styles.container}>
          <Box w={'95%'} h={'95%'} m={15} style={styles.box}>
            <Text style={styles.text}>Bienvenido: {brigade.brigName} {brigade.brigLastname}</Text>
            <Box style={styles.boxCard}>
                ...
                <Image 
                 style={styles.img}
                 source={{uri: 'https://i.gifer.com/origin/ee/ee93dbc6d7115c71190ed0e5e16bfbd6_w200.gif'}}alt='Logo'
                >
                </Image>
               
                
                <Text>{brigade.brigName} {brigade.brigLastname}</Text>
                <Text>Cédula: {brigade.brigDNI}</Text>
            </Box>
            <Box style={styles.butonsPanel}>
                 <Button title="Escanear" style={styles.button}  backgroundColor ={'rgba(46,125,50,255)'}
                     onPress={()=> navigation.navigate('BrigadeScan_Mobile',
                     {name: brigade.brigName, last: brigade.brigLastname, email: email})}
                />
                <Button title="Comentarios" style={styles.button}  backgroundColor ={'rgba(33,150,243,255)'}
                     onPress={()=> navigation.navigate('BrigadeIncident_Mobile',
                     {name: brigade.brigName, last: brigade.brigLastname, email: email})}
                />
            </Box>
            <Button title="Cerrar Sessión" style={styles.button} backgroundColor ={'rgba(211,47,47,255)'}
